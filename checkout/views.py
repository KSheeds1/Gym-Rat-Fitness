...
def cache_checkout_data(request):
    """
    Determine in the webhook whether the user checked the
    save-info box
    """
    try:
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if order_form.is_valid():
            ...
            messages.error(request, 'There was an error with the \
                           form, please doublecheck your \
                           information')
    else:
        bag = request.session.get('bag', {})
        if not bag:
    ...
    if not stripe_public_key:
        messages.warning(request, 'Stripe pub key is missing')
